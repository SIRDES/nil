...
function DetailSkeleton() {
    return (
        <div className="flex-grow animate-pulse">
            <section className="bg-surface-light border-b border-border-light">
                <div className="max-w-7xl mx-auto px-6 py-16 md:py-24">
                    <div className="flex flex-col md:flex-row justify-between gap-8">
                        ...
                        <div className="h-48 w-full md:w-80 rounded-xl bg-slate-200" />
                    </div>
                </div>
            </section>
        </div>
...
export default function ProgramDetail() {
    ...
    return (
        <div className="flex-grow">
            {/* Hero Section */}
            <section className="relative overflow-hidden bg-surface-light border-b border-border-light">
                ...
                <div className="max-w-7xl mx-auto px-6 py-16 md:py-24 relative z-10">
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
                        <div className="flex flex-col gap-6 max-w-2xl">
                            ...
                            {!program.isRegistrationOpen && (
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-red-100 text-red-600 text-xs font-bold uppercase tracking-wider w-fit">
                            ...
                            <div className="flex flex-wrap items-center gap-4 mt-2">
                                <div className="flex items-center gap-2 px-4 py-2 bg-background-light rounded-lg border border-border-light">
                                    <span className="material-symbols-outlined text-primary">schedule</span>
                                    <span className="text-sm font-semibold text-text-main ">{program.duration}</span>
                                </div>
                                {program.difficultyLevel && (
                                    <div className="flex items-center gap-2 px-4 py-2 bg-background-light rounded-lg border border-border-light">
                                        <span className="material-symbols-outlined text-primary">signal_cellular_alt</span>
                                        <span className="text-sm font-semibold text-text-main ">{program.difficultyLevel}</span>
                                    </div>
                                )}
                                <div className="flex items-center gap-2 px-4 py-2 bg-background-light rounded-lg border border-border-light">
                                    <span className="material-symbols-outlined text-primary">workspace_premium</span>
                                    <span className="text-sm font-semibold text-text-main ">Certificate Included</span>
                                </div>
                            </div>
                        </div>
            ...
            <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 lg:grid-cols-12 gap-12">
                ...
                <div className="lg:col-span-8 flex flex-col gap-16">
                    ...
                    {program.curriculumHighlights && program.curriculumHighlights.length > 0 && (
                        <section>
                            <div className="flex items-center gap-3 mb-8">
                                ...
                                <h2 className="text-3xl font-bold text-text-main ">Curriculum Highlights</h2>
                            </div>

                            <div className="bg-white rounded-2xl border border-border-light overflow-hidden">
                                <div className="divide-y divide-border-light">
                                    {program.curriculumHighlights.map((item, index) => (
                                        <label key={index} className="flex items-start gap-4 p-5 hover:bg-background-light cursor-pointer transition-colors group">
                                            <div className="relative flex items-center mt-0.5">
                                                <input type="checkbox" checked={true} disabled className="peer h-6 w-6 rounded border-gray-300 text-primary focus:ring-primary/20" />
                                            </div>
                                            <div>
                                                <p className="text-base font-semibold text-text-main group-hover:text-primary transition-colors">{item}</p>
                                            </div>
                                        </label>
                                    ))}
                                </div>
                    ...
                    <section>
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <h2 className="text-3xl font-bold text-text-main ">About This Program</h2>
                        </div>
                ...
                <div className="lg:col-span-4 flex flex-col gap-8">
                    ...
                    {/* Sticky CTA */}
                    <div className="sticky top-24 z-10 bg-surface-light p-6 rounded-xl shadow-lg border border-primary/20">
                        <div className="flex flex-col gap-3">
                            ...
                            {program.isFinancialAidEligible && (
                                <p className="text-xs text-emerald-600 text-center font-medium">Flexible payment plans and scholarships available.</p>
                            )}
                            {program.isRegistrationOpen ? (
