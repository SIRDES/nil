...
export async function PUT(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    // If a new banner is being uploaded, delete the old one
    if (body.bannerPublicId) {
      const existingProgram = await Program.findById(id);
      if (existingProgram?.bannerPublicId && existingProgram.bannerPublicId !== body.bannerPublicId) {
        try {
          await deleteImage(existingProgram.bannerPublicId);
        } catch (delError) {
...
export async function DELETE(_request: NextRequest, { params }: RouteParams) {
  try {
    ...
    if (program.bannerPublicId) {
      try {
        await deleteImage(program.bannerPublicId);
      } catch (delError) {
        console.error('Failed to delete image on program deletion:', delError);
        // We already deleted the program from DB, so just log the error
      }
    }
