...
function getCategoryStyle(cat?: string): { gradient: string; icon: string; pillColor: string } {
  ...
  if (lower.includes("bootcamp") || lower.includes("developer")) return { gradient: "from-violet-500 to-purple-600", icon: "code", pillColor: "bg-purple-100 text-purple-700 dark:bg-purple-400/10 dark:text-purple-400" };
  if (lower.includes("advanced") || lower.includes("ai")) return { gradient: "from-blue-500 to-cyan-500", icon: "psychology", pillColor: "bg-blue-100 text-blue-700 dark:bg-blue-400/10 dark:text-blue-400" };
  if (lower.includes("security")) return { gradient: "from-red-500 to-orange-500", icon: "security", pillColor: "bg-red-100 text-red-700 dark:bg-red-400/10 dark:text-red-400" };
  if (lower.includes("design") || lower.includes("creative")) return { gradient: "from-pink-500 to-rose-500", icon: "palette", pillColor: "bg-pink-100 text-pink-700 dark:bg-pink-400/10 dark:text-pink-400" };
  if (lower.includes("cloud") || lower.includes("infrastructure")) return { gradient: "from-amber-400 to-orange-500", icon: "cloud", pillColor: "bg-amber-100 text-amber-700 dark:bg-amber-400/10 dark:text-amber-400" };
  if (lower.includes("data")) return { gradient: "from-emerald-500 to-teal-500", icon: "insights", pillColor: "bg-emerald-100 text-emerald-700 dark:bg-emerald-400/10 dark:text-emerald-400" };
  if (lower.includes("adult") || lower.includes("education")) return { gradient: "from-indigo-500 to-blue-600", icon: "school", pillColor: "bg-indigo-100 text-indigo-700 dark:bg-indigo-400/10 dark:text-indigo-400" };
  if (lower.includes("junior") || lower.includes("beginner")) return { gradient: "from-sky-400 to-blue-500", icon: "child_care", pillColor: "bg-sky-100 text-sky-700 dark:bg-sky-400/10 dark:text-sky-400" };
  return { gradient: "from-slate-500 to-slate-600", icon: "menu_book", pillColor: "bg-slate-100 text-slate-700 dark:bg-slate-400/10 dark:text-slate-400" };
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={refetch} className="h-9 px-2 rounded-lg border border-slate-200 dark:border-slate-700 text-slate-400 hover:text-slate-600 transition-all" title="Refresh">
            ...
            return (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <h3 className="text-base font-bold text-slate-900 dark:text-white mb-1.5">{prog.name}</h3>
                  ...
                  <div className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800 grid grid-cols-3 gap-2 text-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-slate-900 dark:text-white mt-0.5">{prog.activeStudents}</p>
                    </div>
                    <div>
                      <p className="text-xs text-slate-400 dark:text-slate-500 font-medium">Price</p>
                      <p className="text-sm font-bold text-slate-900 dark:text-white mt-0.5">${prog.price.toLocaleString()}</p>
                    </div>
                  </div>
                </div>
              </div>
