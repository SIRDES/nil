...
function getProgramPrice(reg: Registration): string {
    if (typeof reg.programId === "object" && reg.programId !== null)
        return `GHS ${reg.programId.price.toLocaleString()}`;
    return "--";
}
...
function StatusPill({ status }: { status: string }) {
    ...
    return (
        <span
            className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold ring-1 ring-inset ${classes[status] || "bg-slate-100 text-slate-600 ring-slate-400/20"
                }`}
        >
            {status}
        </span>
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-black text-slate-900 ">Registrations</h1>
            ...
            <div className="flex items-center gap-4">
                <select
                    ...
                    className="h-10 px-4 rounded-lg border border-slate-200 bg-white text-sm text-slate-700 focus:ring-2 focus:ring-primary"
                >
                ...
                <button
                    onClick={refetch}
                    className="h-10 px-3 rounded-lg border border-slate-200 bg-white text-slate-400 hover:text-slate-600 transition-all"
                    title="Refresh"
                >
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-slate-100 bg-slate-50">
                                    <th className="text-left px-6 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 ">Student</th>
                                    ...
                                    <th className="text-left px-6 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 ">Payment</th>
                                    <th className="text-left px-6 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 ">Status</th>
                                    <th className="text-left px-6 py-3 text-xs font-bold uppercase tracking-wider text-slate-500 ">Actions</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                                {filtered.map((r) => (
                                    <tr key={r._id} className="hover:bg-slate-50/30 transition-colors">
                                        ...
                                        <td className="px-4 py-2 text-xs text-slate-600 ">{getProgramName(r)}</td>
                                        <td className="px-4 py-2 text-xs text-slate-500 ">{formatDate(r.createdAt)}</td>
                                        <td className="px-4 py-2 text-xs font-semibold text-slate-900 ">{getProgramPrice(r)}</td>
                                        <td className="px-4 py-2">
                                            <span
                                                className={`inline-flex items-center gap-1.5 text-xs font-semibold ${r.paymentReceived ? "text-green-600" : "text-amber-600"
                                                    }`}
                                            >
                ...
                {!loading && !error && filtered.length > 0 && (
                    <div className="flex items-center justify-between px-4 py-2 border-t border-slate-200">
