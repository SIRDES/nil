    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-black text-slate-900 dark:text-white">Instructor Profile</h1>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Viewing details for {instructor.firstName} {instructor.lastName}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 flex flex-col items-center text-center">
            <div className="size-32 rounded-full overflow-hidden mb-4 border-4 border-slate-50 dark:border-slate-800">
            ...
            <h2 className="text-xl font-bold text-slate-900 dark:text-white">{instructor.firstName} {instructor.lastName}</h2>
            <p className="text-sm font-medium text-primary mt-1">{instructor.professionalTitle}</p>
            
            <div className="mt-4 flex items-center gap-2">
              <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
                instructor.availabilityStatus === "Active"
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <a href={`mailto:${instructor.email}`} className="text-sm font-medium text-slate-900 dark:text-white hover:text-primary transition-colors">{instructor.email}</a>
                </div>
              </div>
              ...
              {instructor.phone && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <a href={`tel:${instructor.phone}`} className="text-sm font-medium text-slate-900 dark:text-white hover:text-primary transition-colors">{instructor.phone}</a>
                  </div>
                </div>
              )}
              ...
              {instructor.location && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-slate-900 dark:text-white">{instructor.location}</p>
                  </div>
                </div>
              )}
              ...
              {instructor.linkedInUrl && (
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">LinkedIn Profile</p>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6">
            ...
            {instructor.assignedPrograms && instructor.assignedPrograms.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {instructor.assignedPrograms.map((prog) => (
                  <Link 
                    key={prog._id} 
                    href={`/admin/programs`} 
                    className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-primary dark:hover:border-primary hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group"
                  >
              ...
              <p className="text-sm text-slate-400 italic bg-slate-50 dark:bg-slate-800/50 p-4 rounded-lg border border-dashed border-slate-200 dark:border-slate-700 text-center">
                This instructor is not assigned to any programs yet.
              </p>
            )}
          </div>
          
          <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 text-sm text-slate-500 dark:text-slate-400">
