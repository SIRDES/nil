...
export default function EditInstructorPage({ params }: { params: Promise<{ id: string }> }) {
 ...
 <div>
 <h1 className="text-2xl font-black text-slate-900">Edit Instructor</h1>
 <p className="text-sm text-slate-500 mt-0.5">Update the instructor profile and program assignments.</p>
 </div>
 </div>
 ...
 <div>
 <p className="text-sm font-semibold text-slate-900 ">Send Welcome Email</p>
 <p className="text-xs text-slate-500 mt-0.5">Automatically send login credentials to the new instructor.</p>
 </div>
 <button
 type="button"
 role="switch"
 ...
 form.sendWelcomeEmail ? "bg-primary" : "bg-slate-300"
 }`}
 >
 ...
 className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
 form.publicProfile ? "bg-primary" : "bg-slate-300"
 }`}
 >
